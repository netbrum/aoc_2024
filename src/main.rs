...
fn is_safe(row: &[i8]) -> bool {
    let diffs: Vec<i8> = row.windows(2).map(|w| w[0] - w[1]).collect();

    let increasing = diffs.iter().all(|d| *d >= 1 && *d <= 3);
    let decreasing = diffs.iter().all(|d| *d <= -1 && *d >= -3);

    increasing || decreasing
}

fn main() {
    let safe: usize = INPUT.iter().map(|row| usize::from(is_safe(row))).sum();
    let tolerable: usize = INPUT
        .iter()
        .map(|row| {
            if is_safe(row) {
                return 1;
    ...
    println!("Total: {safe}");
    println!("Total with tolerance: {tolerable}");
}
